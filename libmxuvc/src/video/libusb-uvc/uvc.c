...
enum AVC_XU_CTRL {
	...
	AVC_XU_NUM_CTRLS = AVC_XU_MAX_FRAME_SIZE,
};
...
enum PU_XU_CTRL {
	...
	PU_XU_NUM_CTRLS = PU_XU_GAIN_MULTIPLIER,
};
...
static void capture_callback(struct libusb_transfer *transfer)
{
	...
	if (vcfg->vs.bulk) {
		static unsigned char framebuf1[NUM_VID_CHANNEL][MAX_FRAME_SIZE];
		static unsigned char framebuf2[NUM_VID_CHANNEL][MAX_FRAME_SIZE];
		static unsigned char *framebuf[NUM_VID_CHANNEL];
		...
		if(transfer->actual_length != transfer->length ||
			...
			if(framebuf[ch] == framebuf1[ch])
				framebuf[ch] = framebuf2[ch];
			else
				framebuf[ch] = framebuf1[ch];
			framelen[ch] = 0;
		}
...
static int alloc_transfers(video_channel_t ch, int timeout)
{
	...
	if(vcfg->vs.bulk) {
		/* 16 kB is the maximum URB size that libusb can ask the
		 * kernel for. Bigger size will be split by libusb into
		 * 16 kB ones. */
		if(vset->max_transfer_size < 16384)
			usb_packet_size = vset->max_transfer_size;
		else
			usb_packet_size = 16384;

		//usb_packet_size = vcfg->vs.alt_maxpacketsize[vset->cur_alt];
...
int mxuvc_video_init(const char *backend, const char *options)
{
	...
	TRACE("Using num_transfers = %u\n",          num_transfers);
	TRACE("Using check_h264_continuity = %i\n",  check_h264_continuity);
	...
	TRACE("Opening USB device %x:%x for video\n", vendor_id, product_id);
	video_hdl = libusb_open_device_with_vid_pid(video_ctx, vendor_id,
							product_id);
	CHECK_ERROR(video_hdl == NULL, -1, "Could not open USB device "
...
int mxuvc_video_start(video_channel_t ch)
{
	...
	TRACE("Starting video on %s channel.\n", chan2str(ch));

	vset = get_vset(ch);
	vcfg = get_vcfg(ch);
	CHECK_ERROR(vcfg->enabled == 0, -1,
			"Unable to start the video. %s channel is not "
			"enabled.", chan2str(ch));

	CHECK_ERROR(vset->started == 1, -1, "Unable to start the video on %s "
			"channel: video has already been started.",
			chan2str(ch));

	CHECK_ERROR(video_disconnected != 0, -1,
			"Unable to start the video on %s "
			"channel: the camera has been disconnected.",
			chan2str(ch));

	/* Probe/Commit to find the alternate setting corresponding
	 * to the default format/frame */
	ret = probe_commit_settings(ch);
	CHECK_ERROR(ret < 0, -1, "Failed to probe/commit the video");
	...
	if(!vcfg->vs.bulk) {
		TRACE2("Setting alternate settings to %i for interface %i\n",
			vset->cur_alt,
			vcfg->vs.interface);
		ret = libusb_set_interface_alt_setting(video_hdl,
			vcfg->vs.interface, vset->cur_alt);
		CHECK_ERROR(ret < 0, -1,
			"Unable to start the video: could not set the "
			"alternate setting.");
	}
	...
	for (i=0; i<num_transfers; i++) {
		...
		if(ret < 0) {
			...
			ERROR(-1, "Unable to start the video: cannot submit "
				"the USB transfer");
		}
		incr_active_transfers(ch);
	}
...
int mxuvc_video_stop(video_channel_t ch)
{
	...
	TRACE("Stopping video on %s channel.\n", chan2str(ch));

	vset = get_vset(ch);
	vcfg = get_vcfg(ch);
	CHECK_ERROR(vcfg->enabled == 0, -1,
			"Unable to stop the video. %s channel is not "
			"enabled.", chan2str(ch));

	CHECK_ERROR(vset->started == 0, -1, "Unable to stop the video on %s "
			"channel: video hasn't been started yet.",
			chan2str(ch));
	...
	if (!video_disconnected) {
		...
		int ret;
		/* Set alternate settings to 0 on video streaming interface */
		TRACE2("Setting alternate settings to 0 for interface %i\n",
				vcfg->vs.interface);
		ret = libusb_set_interface_alt_setting(video_hdl,
				vcfg->vs.interface, 0);
		CHECK_ERROR(ret < 0, -1,
				"Unable to stop the video: could not set the "
				"alternate setting.");
	} else {
		while(get_active_transfers(ch))
			usleep(3000);
...
int mxuvc_video_register_cb(video_channel_t ch, mxuvc_video_cb_t func, void* user_data)
{
	...
	vcfg = get_vcfg(ch);
	CHECK_ERROR(vcfg->enabled == 0, -1,
			"Unable to register video callback function for %s "
...
int mxuvc_video_set_format(video_channel_t ch, video_format_t fmt)
{
	...
	TRACE("Setting video format to %s on %s channel.\n",
			vidformat2str(fmt), chan2str(ch));

	vset = get_vset(ch);
	vcfg = get_vcfg(ch);
	CHECK_ERROR(vcfg->enabled == 0, -1,
			"Unable to set video format. %s channel is not "
			"enabled", chan2str(ch));

	/* Return if we are already using the requested format */
	...
	case VID_FORMAT_NV12_RAW:
		...
		break;
	default:
		ERROR(-1, "Enable to set video format to unknown format %i", fmt);
	}
	...
	CHECK_ERROR(found == 0, -1, "Format %s not found for %s channel",
			vidformat2str(fmt), chan2str(ch));
	...
	if((vset->cur_format->type == VS_FORMAT_MPEG2TS) &&
		...
		CHECK_ERROR(ret < 0, -1, "Unable to %s AAC muxing on %s channel.",
				value == 0 ? "disable" : "enable", chan2str(ch));
...
int mxuvc_video_set_resolution(video_channel_t ch, uint16_t width, uint16_t height)
{
	...
	TRACE("Setting resolution to %ix%i on %s channel.\n",
			width, height, chan2str(ch));

	vset = get_vset(ch);
	vcfg = get_vcfg(ch);
	CHECK_ERROR(vcfg->enabled == 0, -1,
			"Unable to set resolution. %s channel is not "
			"enabled.", chan2str(ch));
	...
	case VID_FORMAT_H264_TS:
	case VID_FORMAT_H264_AAC_TS:
		//CHECK_ERROR(width > 0xffff || height > 0xffff, -1,
		//	"Unable to set resolution. Width and height must less "
		//	"than %i.", 0xffff);
		res = (width << 16) + height;
	...
	default:
		...
		CHECK_ERROR(found == 0, -1, "Unable to set resolution. %ix%i "
				"is not available for the current "
				"format (%i).", width, height,
				vset->cur_video_format);
