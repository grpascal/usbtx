...
//	uint32_t framerate = 0;
	...
	{
		chn[channel] = channel;
		ret = mxuvc_video_register_cb(channel, ch_cb, &chn[channel]);
		if(ret < 0)
			goto error;
	}
	...
	if(ret < 0)
		goto error;
	printf("CH1 profile %d\n",profile);
	uint32_t avc_level = 0;
	ret = mxuvc_video_get_avc_level(CH1, &avc_level);
	printf("CH1 avc level %d\n",avc_level);
	avc_level = 40;
	ret = mxuvc_video_set_avc_level(CH1, avc_level);
